(function (global_scope) {
    ...
    class CanvasRenderer {
        constructor(canvas) {
            ...
            this.level = null;
            this.board_bounds = { x: 0, y: 0, size: 1, cell_size: 12 };
            this.css_width = 1;
            this.css_height = 1;
            this.fixed_cell_size = 12;
            this.display_scale = 1;
            // The display is CSS-scaled while the backing canvas remains memory capped.
            // This gives players an effectively unrestricted useful zoom range
            // without allocating enormous mobile canvas buffers.
            this.minimum_display_scale = 0.02;
            this.maximum_display_scale = 512;
            this.maximum_canvas_pixels = 9000000;
            this.board_padding = 14;
        ...
        clamp_display_scale(value) {
            ...
            return Math.min(
                this.maximum_display_scale,
                Math.max(this.minimum_display_scale, numeric_value)
            );
        }
        ...
        set_display_scale(value) {
            const next_scale = this.clamp_display_scale(value);

            if (Math.abs(next_scale - this.display_scale) < 0.0001) {
                return false;
            }

            this.display_scale = next_scale;
            this.resize();
            return true;
        }

        get_render_scale(css_width, css_height) {
            const desired_scale = this.get_device_scale() * this.display_scale;
            const logical_area = Math.max(1, css_width * css_height);
            const memory_scale = Math.sqrt(
                this.maximum_canvas_pixels / logical_area
            );

            return Math.max(
                0.75,
                Math.min(desired_scale, memory_scale)
            );
        }

        resize() {
            const columns = this.level ? this.level.columns : 24;
            const rows = this.level ? this.level.rows : 24;
            const cell_size = this.fixed_cell_size;
            const padding = this.board_padding;
            const css_width = Math.max(1, columns * cell_size + padding * 2);
            const css_height = Math.max(1, rows * cell_size + padding * 2);
            const render_scale = this.get_render_scale(css_width, css_height);
            const display_width = css_width * this.display_scale;
            const display_height = css_height * this.display_scale;

            this.css_width = css_width;
            this.css_height = css_height;
            this.canvas.width = Math.ceil(css_width * render_scale);
            this.canvas.height = Math.ceil(css_height * render_scale);
            ...
            this.board_bounds = {
                x: padding,
                y: padding,
                size: Math.max(board_width, board_height),
                cell_size
            };
        }
        ...
        pointer_to_cell(event) {
            ...
            const scale_x = rect.width / Math.max(1, this.css_width);
            const scale_y = rect.height / Math.max(1, this.css_height);
            const local_x = (event.clientX - rect.left) / Math.max(0.0001, scale_x);
            const local_y = (event.clientY - rect.top) / Math.max(0.0001, scale_y);
        ...
        render(session, visual_state = {}) {
            ...
            const render_scale = this.canvas.width / Math.max(1, this.css_width);
            ...
            if (!this.level) {
                ...
                context.fillRect(0, 0, this.css_width, this.css_height);
                context.restore();
                return;
            }
        ...
        ensure_background_cache() {
            ...
            const render_scale =
                this.background_canvas.width / Math.max(1, this.css_width);
